...
void dfs(vector<string>& grid,vector<vector<bool>>&vis,int i,int j,int &area,int &perim){
    int n=grid.size();
    area++;
...
void solve() {
   ...
   for(int i=0;i<n;i++){
       for(int j=0;j<n;j++){
           if(grid[i][j]=='#' && !vis[i][j]){
            //   dfs(grid,vis,i,j,area,perim);
               vis[i][j]=true;
               int area=0;
               int perim=0;
               //cout<<maxarea<<" "<<minperim<<endl;
               dfs(grid,vis,i,j,area,perim);
               if(maxarea<area){
               maxarea=area;
               minperim=perim;
