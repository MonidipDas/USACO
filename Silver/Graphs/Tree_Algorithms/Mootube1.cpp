...
void dfs(vector<vector<pair<int,int>>>& adj,int node,int par,int k,multiset<int>& ms,int &cnt){
    //st.insert(node);
    cnt++;
    for(auto &[it,wt]:adj[node]){
        if(it!=par){
            ms.insert(wt);
            if(*ms.begin()<k){
                auto addr=ms.find(wt);
                ms.erase(addr);
                continue;
            }
            dfs(adj,it,node,k,ms,cnt);
            auto addr=ms.find(wt);
            ms.erase(addr);
        }
    }
