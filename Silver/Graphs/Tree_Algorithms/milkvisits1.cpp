...
bool dfs(vector<vector<int>>& adj,int node,int par,int target,vector<int>& path){
    path.push_back(node);
    if(node==target){
        return true;
    }
    for(auto it:adj[node]){
        if(it!=par){
            if(dfs(adj,it,node,target,path)){
                return true;
            }
        }
    }
...
void calc(vector<int>& vp,string s,vector<int>& path){
    char ch=s[path[0]];
    int cnt=0;
    vp.push_back(0);
    for(int i=1;i<path.size();i++){
        if(s[path[i-1]]!=s[path[i]]){
            cnt++;
            vp.push_back(cnt);
        }
    }
}
// for upto tc 5 O(n*m) solution
void solve() {
    int n,m;
    cin>>n>>m;
    string s;
    ...
    for(int i=0;i<m;i++){
        ...
        vector<int>change;
        calc(change,s,p);
        int val1=*change.begin();
        int val2=*change.rbegin();
        if(val1!=val2){
            ans+='1';
        }else{
            if(s[p[0]]==c){
